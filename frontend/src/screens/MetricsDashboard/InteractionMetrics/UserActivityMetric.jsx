  ...
  return (
    <Paper elevation={3} style={{ padding: '20px', height: '100%' }}>
      ...
      <Typography variant="body2" color="textSecondary" paragraph>
        {metrics?.date_range || 'N/A'} • {metrics?.total_interactions?.toLocaleString() || '0'} total interactions
      </Typography>
      ...
      <Box style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '15px' }}>
        ...
        <MetricStat
          ...
          value={metrics?.average_interactions_per_session?.toFixed(1) || '0'}
          color={UNHCR_PALETTE.darkBlue}
        />
        <MetricStat
          label="Error Rate"
          value={`${metrics?.error_rate?.toFixed(1) || '0'}%`}
          color={(metrics?.error_rate || 0) > 5 ? '#C6362B' : UNHCR_PALETTE.yellow}
        />
      </Box>
    </Paper>
  );
