...
def get_config() -> Dict[str, Any]:
    ...
    config["headless"] = os.environ.get("PLAYWRIGHT_HEADLESS", "").lower() in ("true", "1", "yes")
    config["slow_mo"] = int(os.environ.get("PLAYWRIGHT_SLOW_MO", config["slow_mo"]))
            ...
            try:
                ...
                if video_path and os.path.exists(video_path):
                    ...
                    new_video_path = os.path.join(
                        config["video_dir"],
                        f"{safe_name}_{uuid.uuid4().hex[:8]}.webm"
                    )
