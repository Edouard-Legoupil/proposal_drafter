...
class ArtifactRunLogger:
    ...
    def log_agent_execution(
        ...
        try:
            with self.engine.begin() as connection:
                ...
                current_retry_count = result[2] if result[2] else 0
                current_tokens_input = result[3] if result[3] else 0
    ...
    def log_output_metrics(
        ...
        try:
            ...
            logger.info(f"Logged output metrics for run {run_id}: "
        ...
        try:
            with self.engine.begin() as connection:
                ...
                connection.execute(
                    ...
                    "run_id": run_id,
                    "export_events": json.dumps(export_events)
                })
    ...
    def log_token_usage(
        ...
        try:
            ...
            logger.info(f"Logged token usage for run {run_id}: "
        ...
        try:
            with self.engine.connect() as connection:
                results = connection.execute(
                    ...
                    """
                ), {"artifact_type": artifact_type, "artifact_id": artifact_id}).fetchall()
            ...
            logger.error(f"Failed to get runs for {artifact_type} {artifact_id}: {e}", exc_info=True)
            raise
            ...
            with self.engine.connect() as connection:
                results = connection.execute(
                    text("""
                        ...
                        LIMIT :limit
                    """
                ), {"agent_name": agent_name, "limit": limit}).fetchall()
