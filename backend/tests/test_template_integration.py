...
def test_load_proposal_template_file_fallback():
    ...
    with patch("backend.core.config._load_template_from_db") as mock_db_load, patch(
        "backend.core.config.get_available_templates"
    ) as mock_get_templates, patch("backend.core.config._find_template_path") as mock_find_path, patch(
...
async def test_template_versioning():
    ...
    version_data = TemplateVersionCreate(
        version_notes="Updated with new sections", template_data={"updated": "template"}
    )
...
async def test_template_audit_logging():
    ...
    update_data = TemplateUpdate(name="Updated Template", description="Updated description", status="active")
