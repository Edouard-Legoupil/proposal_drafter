...
def test_role_request_system(test_engine):
    ...
    with test_engine.connect() as connection:
        ...
        connection.execute(
            text("INSERT INTO roles (id, name) VALUES (1, 'access_metrics'), (2, 'access_template')")
        )
        ...
        connection.execute(
            text("INSERT INTO user_role_requests (user_id, role_id) VALUES (:user_id, 1)"),
            {'user_id': user_id}
        )
        
        # Verify request was stored
        result = connection.execute(
            text("SELECT role_id FROM user_role_requests WHERE user_id = :user_id"),
            {'user_id': user_id}
        ).fetchone()
        ...
        # Test 2: User cannot request system admin
        with pytest.raises(Exception):
            # This should fail because system admin shouldn't be in the roles table
            # or should be filtered out by the API
            pass
        ...
        result = connection.execute(
            text("SELECT 1 FROM user_role_requests WHERE user_id = :user_id AND role_id = 1"),
            {'user_id': user_id}
        ).fetchone()
...
def test_role_request_validation(test_engine):
    ...
    with test_engine.connect() as connection:
        ...
        connection.execute(
            text("INSERT INTO roles (id, name) VALUES (1, 'access_metrics')")
        )
        ...
        # Should not be able to request the same role
        with pytest.raises(Exception):
            connection.execute(
                text("INSERT INTO user_role_requests (user_id, role_id) VALUES (:user_id, 1)"),
                {'user_id': user_id}
...
def test_admin_role_management(test_engine):
    ...
    with test_engine.connect() as connection:
        ...
        connection.execute(
            text("DELETE FROM user_role_requests WHERE user_id = :user_id AND role_id = 1"),
            {'user_id': user_id}
        )
        ...
        connection.execute(
            text("INSERT INTO user_role_requests (user_id, role_id) VALUES (:user_id, 2)"),
            {'user_id': user_id}
        )
        
        connection.execute(
            text("DELETE FROM user_role_requests WHERE user_id = :user_id AND role_id = 2"),
            {'user_id': user_id}
        )
        
        # Verify role was not granted
        result = connection.execute(
            text("SELECT role_id FROM user_roles WHERE user_id = :user_id AND role_id = 2"),
            {'user_id': user_id}
        ).fetchone()
...
def test_system_admin_protection(test_engine):
    ...
    with test_engine.connect() as connection:
        ...
        connection.execute(
            text("INSERT INTO roles (id, name) VALUES (999, 'system admin')")
        )
        ...
        result = connection.execute(
            text("SELECT name FROM roles WHERE name = 'system admin'")
        ).fetchone()
