...
# Internal Modules
from backend.models.team import Team, TeamRole, TeamMember
...
def test_role_inheritance_for_team_members(test_engine):
    ...
    with test_engine.connect() as connection:
        # Create roles
        connection.execute(
            text("INSERT INTO roles (id, name) VALUES (1, 'direct_role'), (2, 'team_role')")
        )
...
def test_user_model_role_inheritance(test_engine):
    ...
    with test_engine.connect() as connection:
        ...
        user_id = str(uuid.uuid4())
        connection.execute(
            text("INSERT INTO users (id, email, password, name, team_id) VALUES (:id, :email, :password, :name, :team_id)"),
            {"id": user_id, "email": "test@example.com", "password": "password", "name": "Test User", "team_id": team_id}
        )
...
def test_permission_checking_with_inherited_roles(test_engine):
    ...
    with test_engine.connect() as connection:
        ...
        assert user.has_permission("access_metrics", connection) == True  # Inherited from team
        assert user.has_permission("access_template", connection) == False  # Not assigned
        assert user.has_permission("access_incident", connection) == False  # Not assigned
...
def test_admin_bypass(test_engine):
    ...
    with test_engine.connect() as connection:
        # Create admin role
        connection.execute(
            text("INSERT INTO roles (id, name) VALUES (1, 'system admin')")
        )
        ...
        assert user.has_permission("access_metrics") == True
        assert user.has_permission("access_template") == True
        assert user.has_permission("nonexistent_permission") == True
