...
def test_settings_request_workflow(test_engine):
    ...
    with test_engine.connect() as connection:
        ...
        connection.execute(text("INSERT INTO donors (id, name) VALUES ('donor-1', 'Test Donor 1')"))
        connection.execute(text("INSERT INTO outcomes (id, name) VALUES ('outcome-1', 'Test Outcome 1')"))
        connection.execute(text("INSERT INTO field_contexts (id, name) VALUES ('field-1', 'Test Field Context 1')"))
        connection.execute(text("INSERT INTO teams (id, name) VALUES ('team-1', 'Test Team 1')"))
        ...
        connection.execute(
            text("INSERT INTO user_donors (user_id, donor_id) VALUES (:user_id, 'donor-1')"), {"user_id": user_id}
        )
        connection.execute(
            text(
                """
        ...
        result = connection.execute(
            text("SELECT donor_id FROM user_donors WHERE user_id = :user_id"), {"user_id": user_id}
        ).fetchone()
...
def test_settings_validation(authenticated_client, db_session):
    """Test settings request validation"""
    user_id = authenticated_client.app.dependency_overrides[get_current_user]()["user_id"]
    db_session.execute(text("INSERT INTO donors (id, name) VALUES ('donor-1', 'Test Donor 1')"))
    db_session.execute(
    ...
    response = authenticated_client.post(
        ...
        json={"setting_type": "donor_focal", "setting_value": "donor-1"},
    )

    assert response.status_code == 400
    assert response.json()["detail"] == "You already have this donor focal"
...
def test_admin_settings_management(test_engine):
    ...
    with test_engine.connect() as connection:
        ...
        result = connection.execute(
            text("SELECT outcome_id FROM user_outcomes WHERE user_id = :user_id"), {"user_id": user_id}
        ).fetchone()
...
def test_settings_request_types(test_engine):
    ...
    with test_engine.connect() as connection:
        # Setup: Create test data
        connection.execute(text("INSERT INTO donors (id, name) VALUES ('donor-1', 'Test Donor')"))
        connection.execute(text("INSERT INTO outcomes (id, name) VALUES ('outcome-1', 'Test Outcome')"))
        connection.execute(text("INSERT INTO field_contexts (id, name) VALUES ('field-1', 'Test Field Context')"))
