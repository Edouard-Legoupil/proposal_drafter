...
class TemplateSynchronizer:
    ...
    def __init__(self, dry_run: bool = False, verbose: bool = False, 
                 remove_deprecated: bool = False, flush: bool = False,
    ...
    def create_registry(self, template_key: str, template_name: str, 
                       template_type: str, description: str = "") -> Optional[str]:
        ...
        try:
            with conn.cursor() as cur:
                ...
                conn.commit()
                result = cur.fetchone()
                self.log(f"Created registry: {template_key} (type: {template_type})", "success")
    ...
    def create_version(self, registry_id: str, template_data: Dict,
                       version_number: str,
                       environment: str = "uat") -> Optional[str]:
        ...
        try:
            with conn.cursor() as cur:
                ...
                cur.execute(query, (
                    ...
                    json.dumps(template_data),
                    f"Auto synced from filesystem",
                    self.system_user_id,
                    self.system_user_id
                ))
                conn.commit()
                self.log(f"Created version {version_number} for registry {registry_id}", "success")
                return version_id
        except Exception as e:
            if conn:
                conn.rollback()
    ...
    def remove_registry(self, registry_id: str, template_key: str) -> bool:
        ...
        if self.dry_run:
            self.log(f"[DRY RUN] Would remove registry {template_key} ({registry_id})", "info")
            return True
    ...
    def synchronize_template(self, file_template: Dict) -> bool:
        ...
        checksum = file_template['checksum']
        ...
        if not registry:
            ...
            registry_id = self.create_registry(
                template_key, template_name, template_type, description
            )
            if not registry_id:
                return False
            ...
            version_id = self.create_version(
                registry_id, template_data, "1", "uat"
            )
            if version_id:
            ...
            if not latest_version:
                ...
                version_id = self.create_version(
                    registry['id'], template_data, "1", "uat"
                )
                if version_id:
                    self.templates_added += 1
...
def main():
    ...
    parser.add_argument('--system-user-id',
