...
class TemplateValidator:
    ...
    def validate_template_type(self, data: Dict) -> bool:
        ...
        if self.verbose:
            self.add_result(
                ...
                "Template Type",
                f"Valid template_type: {template_type}",
            )
        return True
    ...
    def validate_donors(self, data: Dict) -> bool:
        ...
        if len(donors) == 0:
            ...
            self.add_result(
                ValidationLevel.INFO,
                "Donors",
                f"Found {len(donors)} donor(s): {', '.join(donors[:3])}",
            )
    ...
    def validate_sections(self, data: Dict) -> bool:
        ...
        for i, section in enumerate(sections):
            ...
            if "instructions" not in section and format_type not in [
                "fixed_text",
                "table",
            ]:
                self.add_result(
    ...
    def validate_all_templates(self) -> Tuple[int, int, int, int]:
        ...
        template_files = sorted(self.templates_dir.glob("proposal_template_*.json")) + sorted(
            self.templates_dir.glob("concept_note_*.json")
        )
        ...
        print(
            f"Invalid templates:   {Colors.RED if (total_count - valid_count) > 0 else Colors.GREEN}{total_count - valid_count}{Colors.RESET}"
        )
        print(f"Total errors:        {Colors.RED if total_errors > 0 else Colors.GREEN}{total_errors}{Colors.RESET}")
        print(
            f"Total warnings:      {Colors.YELLOW if total_warnings > 0 else Colors.GREEN}{total_warnings}{Colors.RESET}"
        )
