...
def populate_database(cur, user_id, excel_file):
    ...
    def upsert_knowledge_card(entity_type, entity_name, entity_id, template_name):
        ...
        knowledge_card_ids[
            (
                entity_type if entity_type != "field_context" else "field_contexts",
                entity_name,
            )
        ] = card_id
...
def main():
    parser = argparse.ArgumentParser(
        description="Populate the database with knowledge cards and references from an Excel file."
    )
    ...
    parser.add_argument(
        ...
        default="update",
        help="Choose 'update' to add new data or 'reset' to delete all existing data before populating.",
    )
