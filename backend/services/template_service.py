...
class TemplateService:
            ...
            try:
                async with conn.transaction():
                    ...
                    await self._log_template_action(
                        conn, template_id, version_row["id"], 
                        "template_created", {"initial_version": "1.0"}, user_id
                    )
    ...
    async def update_template_metadata(self, template_id: uuid.UUID, 
            ...
            try:
                async with conn.transaction():
                    ...
                    if update_data.is_default is not None:
                        ...
                        values.append(update_data.is_default)
                        param_index += 1
                    ...
                    # Log the update
                    await self._log_template_action(
                        conn, template_id, None, 
                        "template_updated", dict(update_data), user_id
                    )
    ...
    async def create_template_version(self, template_id: uuid.UUID,
        ...
        async with self.db_pool.acquire() as conn:
            try:
                async with conn.transaction():
                    ...
                    await self._log_template_action(
                        conn, template_id, row["id"], 
                        "version_created", {"version_number": new_version}, user_id
                    )
    ...
    async def _log_template_action(self, conn, template_id: uuid.UUID, 
                                    version_id: Optional[uuid.UUID], 
                                    action: str, action_details: Dict[str, Any],
