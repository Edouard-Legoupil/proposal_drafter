...
async def get_available_settings(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch available settings: {str(e)}") from e


@router.post("/settings/requests")
async def request_setting(request_data: dict, current_user: dict = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if not setting_type or not setting_value:
            raise HTTPException(status_code=400, detail="setting_type and setting_value are required")
        ...
        with get_engine().begin() as connection:
            ...
            if setting_type == "donor_focal":
                ...
                exists = connection.execute(
                    text(
                        "SELECT 1 FROM user_field_contexts WHERE user_id = :user_id "
                        "AND field_context_id = :field_context_id"
                    ),
            ...
            if exists:
                raise HTTPException(status_code=400, detail=f"You already have this {setting_type.replace('_', ' ')}")
            ...
            pending = connection.execute(
                ...
                {"user_id": current_user["user_id"], "setting_type": setting_type, "setting_value": setting_value},
            ).fetchone()

            if pending:
                raise HTTPException(status_code=400, detail="You already have a pending request for this setting")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to submit setting request: {str(e)}") from e
...
async def get_pending_settings_requests(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch pending settings requests: {str(e)}") from e


@router.delete("/settings/requests/{request_id}")
async def cancel_setting_request(request_id: int, current_user: dict = Depends(get_current_user)):
    """
    ...
    try:
        with get_engine().begin() as connection:
            ...
            # Delete the request
            connection.execute(
                text("DELETE FROM user_settings_requests WHERE id = :request_id"), {"request_id": request_id}
            )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to cancel setting request: {str(e)}") from e
