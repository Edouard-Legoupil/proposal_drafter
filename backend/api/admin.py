...
from backend.models.schemas import Role, CreateTeamRequest, UpdateUserTeamRequest
...
async def create_team(request: CreateTeamRequest, admin: dict = Depends(is_system_admin)):
    ...
    try:
        with get_engine().begin() as connection:
            ...
            existing = connection.execute(text("SELECT id FROM teams WHERE lower(name) = :name"), {"name": request.name.lower()}).fetchone()
            if existing:
            ...
            return {"message": "Team created successfully.", "team": {"id": team_id, "name": request.name}}
    except HTTPException:
        raise
    except Exception as e:
...
async def update_user_team(user_id: str, request: UpdateUserTeamRequest, admin: dict = Depends(is_system_admin)):
    ...
    try:
        with get_engine().begin() as connection:
            ...
            # Verify team exists
            team_check = connection.execute(text("SELECT id FROM teams WHERE id = :team_id"), {"team_id": str(request.team_id)}).fetchone()
            if not team_check:
