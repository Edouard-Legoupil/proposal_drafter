    ...
    try:
        ...
        with get_engine().connect() as connection:
            ...
            if exists:
                raise HTTPException(
                    status_code=400,
                    detail="This setting already exists for the team"
                )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to add team setting: {str(e)}"
        )

@router.delete("/admin/team-settings/{setting_id}")
async def remove_team_setting(
    setting_id: int,
    admin: dict = Depends(is_system_admin)
):
    """
    ...
    try:
        with get_engine().connect() as connection:
            ...
            if not setting:
                raise HTTPException(
                    status_code=404,
                    detail="Team setting not found"
                )
            
            # Delete the setting
            connection.execute(
                text("DELETE FROM team_settings WHERE id = :setting_id"),
                {'setting_id': setting_id}
            )
            
            # Remove inherited settings from team members
            connection.execute(
                text("""
                DELETE FROM user_settings_requests 
                WHERE user_id IN (
            ...
            return {
                "message": "Team setting removed successfully",
                "setting_id": setting_id
            }
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to remove team setting: {str(e)}"
        )

@router.get("/admin/teams/{team_id}/effective-settings")
async def get_team_effective_settings(
    team_id: str,
    admin: dict = Depends(is_system_admin)
):
    """
    ...
    try:
        with get_engine().connect() as connection:
            ...
            return {
                "team_id": team_id,
                "team_settings": team_settings_list,
                "team_members": members_list
            }
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch team effective settings: {str(e)}"
        )