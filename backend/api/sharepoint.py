...
async def upload_document_to_sharepoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to upload document to SharePoint: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            auth_logger.info(
                ...
                extra={
                    ...
                    "action": "sharepoint_upload_proposal",
                    "result": "allowed",
                },
            )
        except HTTPException as auth_exc:
            auth_logger.warning(
                "Proposal SharePoint upload denied",
                extra={
                    "user_id": user_id,
                    "proposal_id": str(proposal_uuid),
                    "action": "sharepoint_upload_proposal",
        ...
        if existing_link:
            ...
            if existing_link["status"] == "failed" and existing_link.get("retry_count", 0) >= MAX_RETRY_ATTEMPTS:
                ...
                raise HTTPException(
                    status_code=429,
                    detail="Max retry attempts exceeded for this document. Please try again later.",
                )
        ...
        try:
            ...
            if should_retry:
                logger.warning(
                    f"SharePoint upload failed for proposal {proposal_id}, retry {new_retry_count}/{MAX_RETRY_ATTEMPTS}"
                )
                raise HTTPException(
                    status_code=503,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to upload proposal to SharePoint: {str(e)}")
...
async def upload_knowledge_card_to_sharepoint(
    ...
    try:
        ...
        # T101: Object-Level Authorization - Verify user has access to this knowledge card
        # Fetch the card's donor_id, outcome_id, field_context_id to check permissions
        card_info = None
        with get_engine().connect() as connection:
            card_info = connection.execute(
                text(
                    """
                    SELECT donor_id, outcome_id, field_context_id, created_by
        ...
        if card_info:
            ...
            try:
                ...
                auth_logger.info(
                    "Knowledge card SharePoint upload authorized",
                    extra={
                        "user_id": user_id,
                        "card_id": str(card_uuid),
                        "action": "sharepoint_upload_knowledge_card",
                        "result": "allowed",
                    },
                )
            except HTTPException as auth_exc:
                auth_logger.warning(
                    "Knowledge card SharePoint upload denied",
                    extra={
                        "user_id": user_id,
                        "card_id": str(card_uuid),
                        "action": "sharepoint_upload_knowledge_card",
        ...
        try:
            ...
            if should_retry:
                logger.warning(
                    f"SharePoint upload failed for knowledge card {card_id}, retry {new_retry_count}/{MAX_RETRY_ATTEMPTS}"
                )
                raise HTTPException(
                    status_code=503,
                    detail=f"Upload failed but will retry. Attempt {new_retry_count}/{MAX_RETRY_ATTEMPTS}. Error: {error_message}",
...
async def retry_sharepoint_upload(
    artifact_type: str, artifact_id: str, current_user: dict = Depends(get_current_user)
) -> JSONResponse:
    """
    ...
    # T102: Object-Level Authorization - Validate artifact type and verify access
    ...
    if artifact_type == "proposal":
        try:
            ...
            auth_logger.info(
                ...
                extra={
                    ...
                    "action": "sharepoint_retry_proposal",
                    "result": "allowed",
                },
            )
        except HTTPException as auth_exc:
            auth_logger.warning(
                "Proposal SharePoint retry denied",
                extra={
                    "user_id": user_id,
                    "proposal_id": str(artifact_uuid),
                    "action": "sharepoint_retry_proposal",
        ...
        # Fetch card info to check permissions
        card_info = None
        with get_engine().connect() as connection:
            card_info = connection.execute(
                text(
        ...
        if card_info:
            ...
            try:
                ...
                auth_logger.info(
                    "Knowledge card SharePoint retry authorized",
                    extra={
                        "user_id": user_id,
                        "card_id": str(artifact_uuid),
                        "action": "sharepoint_retry_knowledge_card",
                        "result": "allowed",
                    },
                )
            except HTTPException as auth_exc:
                auth_logger.warning(
                    "Knowledge card SharePoint retry denied",
                    extra={
                        "user_id": user_id,
                        "card_id": str(artifact_uuid),
                        "action": "sharepoint_retry_knowledge_card",
            ...
            raise HTTPException(status_code=404, detail=f"Knowledge card {artifact_id} not found")
...
async def get_sharepoint_link_status(
    ...
    # T103: Object-Level Authorization - Verify user has access to the artifact
    ...
    if artifact_type == "proposal":
        try:
            ...
            auth_logger.info(
                ...
                extra={
                    "user_id": user_id,
                    "proposal_id": str(artifact_uuid),
                    "action": "sharepoint_status_proposal",
                    "result": "allowed",
                },
            )
        except HTTPException as auth_exc:
            auth_logger.warning(
                "Proposal SharePoint status check denied",
                extra={
                    "user_id": user_id,
                    "proposal_id": str(artifact_uuid),
                    "action": "sharepoint_status_proposal",
        ...
        if card_info:
            ...
            try:
                ...
                auth_logger.info(
                    "Knowledge card SharePoint status check authorized",
                    extra={
                        "user_id": user_id,
                        "card_id": str(artifact_uuid),
                        "action": "sharepoint_status_knowledge_card",
                        "result": "allowed",
                    },
                )
            except HTTPException as auth_exc:
                auth_logger.warning(
                    "Knowledge card SharePoint status check denied",
                    extra={
                        "user_id": user_id,
                        "card_id": str(artifact_uuid),
                        "action": "sharepoint_status_knowledge_card",
    ...
    return JSONResponse(
        ...
        content={
            ...
            "can_retry": existing_link["status"] == "failed"
            and existing_link.get("retry_count", 0) < MAX_RETRY_ATTEMPTS,
